...
(module
  ...
  (func (export "unhandled-2")
    (resume (cont.new (type $k0) (ref.func $f1)))
  )

  (func (export "unhandled-3")
    (block $h (result (ref $k0))
      (resume (event $e2 $h) (cont.new (type $k0) (ref.func $f1)))
      (unreachable)
    )
    (drop)
  )
  ...
  (func (export "handled")
    (block $h (result (ref $k0))
      (resume (event $e1 $h) (cont.new (type $k0) (ref.func $f1)))
      (unreachable)
    )
    (drop)
  )
  ...
  (func (export "uncaught-1")
    (block $h (result (ref $k0))
      (resume (event $e1 $h) (cont.new (type $k0) (ref.func $f2)))
      (unreachable)
    )
    (drop)
  )
  ...
  (func (export "barrier")
    (block $h (result (ref $k0))
      (resume (event $e1 $h) (cont.new (type $k0) (ref.func $f3)))
      (unreachable)
    )
    (resume_throw $exn)
  )

  (elem declare func $f5)
  (func $f5 (param $b i32)
    (try
      (do (suspend $e1))
      (catch_all (if (local.get $b) (then (suspend $e1))))
    )
  )

  (func (export "resume_throw_catch") (param $b i32) (result i32)
    (block $h1 (result (ref $k0))
      (resume (event $e1 $h1)
        (local.get $b) (cont.new (type $k1) (ref.func $f5))
      )
      (return (i32.const 1))
    )
    (block $h2 (param (ref $k0)) (result (ref $k0))
      (resume_throw $exn (event $e1 $h2))
      (return (i32.const 2))
    )
    (return (i32.const 3))
  )
  ...
  (func $nl1 (param $k (ref $k0))
    (resume (local.get $k))
    (resume (local.get $k))
  )
  (func $nl2 (param $k (ref $k0))
    (block $h (result (ref $k0))
  ...
  (func $nl3 (param $k (ref $k0))
    (block $h1 (result (ref $k0))
      (resume (event $e1 $h1) (local.get $k))
      (unreachable)
    )
    (let (local $k' (ref $k0))
      (block $h2 (result (ref $k0))
  ...
  (func $nl4 (param $k (ref $k0))
    (drop (cont.bind (type $k0) (local.get $k)))
    (resume (local.get $k))
  )

  (func (export "non-linear-1")
    (call $nl1 (cont.new (type $k0) (ref.func $r0)))
  )
  (func (export "non-linear-2")
    (call $nl2 (cont.new (type $k0) (ref.func $r1)))
  )
  (func (export "non-linear-3")
    (call $nl3 (cont.new (type $k0) (ref.func $r1)))
  )
  (func (export "non-linear-4")
    (call $nl4 (cont.new (type $k0) (ref.func $r1)))
  )
)
...
(assert_return (invoke "resume_throw_catch" (i32.const 0)) (i32.const 2))
(assert_return (invoke "resume_throw_catch" (i32.const 1)) (i32.const 3))
